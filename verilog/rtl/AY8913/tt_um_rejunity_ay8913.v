...
module tt_um_rejunity_ay8913 #( parameter DA7_DA4_UPPER_ADDRESS_MASK = 4'b0000,
    ...
    input  wire [7:0] data,    // Dedicated inputs - connected to the input switches
    output wire [7:0] raw_sample,   // Dedicated outputs - connected to the 7 segment display
    input  wire [1:0] master_clock_control,
    output wire [3:0] pwms,
    input  wire       bdir,
    input  wire       bc1,
    output wire       DAC_le,
    output wire       DAC_clk,
    ...
    tone_ay #(.PERIOD_BITS(12)) tone_A_generator (
        .clk(clk),
    ...
    tone_ay #(.PERIOD_BITS(12)) tone_B_generator (
        .clk(clk),
        .enable(clk_master_strobe),
        .reset(reset),
    ...
    tone_ay #(.PERIOD_BITS(12)) tone_C_generator (
        .clk(clk),
        .enable(clk_master_strobe),
        .reset(reset),
    ...
    noise_ay #(.PERIOD_BITS(5)) noise_generator (
        .clk(clk),
        .enable(clk_master_strobe),
        .reset(reset),
        .period(noise_period),
    ...
    attenuation_ay #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) attenuation_A ( // @TODO: rename to amplitude to match docs
        .in(channel_A),
        .control(envelope_A ? envelope: amplitude_A),
        .out(volume_A)
        );
    attenuation_ay #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) attenuation_B (
        .in(channel_B),
        .control(envelope_B ? envelope: amplitude_B),
        .out(volume_B)
        );
    attenuation_ay #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) attenuation_C (
        .in(channel_C),
    ...
    wire [11:0] spi_master = volume_A + volume_B + volume_C;
    assign { master_overflow, master } = volume_A + volume_B + volume_C; // sum all channels
    assign raw_sample[MASTER_OUTPUT_BITS-1:0] = 
